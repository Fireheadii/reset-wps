...
if [ ! -f "$SESSION" ] ; then
	touch "$SESSION" ; 
else cat session_shee.lst | sed '/.*".*"/d' && cat session_shee.lst | sed '/.*".*"/d' > $LOGFILE
fi
#
tshark -i $IFACE -n -l subtype probereq -T fields -e wlan.sa -e radiotap.dbm_antsignal -e wlan_mgt.ssid -E quote=d 2> /dev/null > "$TMPFILE" &\
...
-e   -- цель ESSID зонд
-h   -- это справочная информация
-H   -- Продлен помощь
-i   -- интерфейс использовать
-m   -- цель MAC adddress
...
FILES ; 
Location for all files saved from shee.sh: $GRNN$LOC$STD
temporary file where data is written to during scans: $GRNN$TMPFILE$STD
log file      : info is saved to file named;$GRNN Date_Time_log_shee.log$STD
...
f_scan_unknown() {
	clear
	if [ ! -f "$WHITELIST" ] ; then touch "$WHITELIST" ; fi
	if [ ! -f "$TMPFILE" ] ; then touch "$TMPFILE" ; fi	
tshark -i $IFACE -n -l subtype probereq -T fields -e wlan.sa -e radiotap.dbm_antsignal -e wlan_mgt.ssid -E quote=d 2> /dev/null > "$TMPFILE" &\
f_header && echo $BLU">$STD Alert on clients not in whitelist" && echo -e "\r" && printf '%-23s %-20s %-7s %-10s\n' "DATE / TIME"	"MAC ADDRESS" "POWER" "ESSID" &\
tail -f $TMPFILE | while read line ; do 
MAC=$(echo $line | cut -d \" -f 2  | tr '[:lower:]' '[:upper:]')
ESSID=$(echo $line | cut -d \" -f 6)
	if [ "$ESSID" == "" ] ; then ESSID="no essid" ; fi
...
MAC_EXIST=$(grep -wi $MAC "$WHITELIST")
	if [ "$MAC_EXIST" == "" ] ; then 
	TIME=$(date +"%Y-%m-%d %H:%M:%S")
	printf '%-23s %-20s %-7s %-10s\n' "$TIME" "$MAC" "$PWR" "$ESSID"  | tee -a $LOGFILE
	if [ "$SOUND" == "ON" ] ; then paplay "$TUNE" ; fi
	fi
done
}
...
tshark -i $IFACE -n -l subtype probereq -T fields -e wlan.sa -e radiotap.dbm_antsignal -e wlan_mgt.ssid -E quote=d 2> /dev/null > "$TMPFILE" &\
f_header && echo $BLU">$STD Alert on clients in blacklist" && echo -e "\r" && printf '%-23s %-20s %-7s %-10s\n' "DATE / TIME"	"MAC ADDRESS" "POWER" "ESSID" &\
...
MAC_EXIST=$(grep -wi $MAC "$BLACKLIST")
	...
	TIME=$(date +"%Y-%m-%d %H:%M:%S")
	printf '%-23s %-20s %-7s %-10s\n' "$TIME" "$MAC" "$PWR" "$ESSID"
	echo "Info in blacklist; $GRN$MAC_EXIST$STD"
	echo ""
		if [ "$SOUND" == "ON" ] ; then paplay "$TUNE" ; fi
#		if [ "$ZENITY" == "TRUE" ] ; then zenity --info --title="Known MAC seen" --text="\r$MAC \rProbed Essid: $ESSID\rInfo: $MAC_EXIST" --display=:0.0 --timeout=3 ; fi
...
#	echo $GRNN"Zenity Popup(-z)-: $GRN$ZENITY$STD"
#	echo $GRNN"Zenity Notify(-n): $GRN$NOTIFY$STD"
	echo ""
...
#						OPTION FUNCTIONS
########################################################################
while getopts ":ac:de:E:hHi:Ilm:M:nrsuUvz" opt; do
  case $opt in
	a) f_auto ;;
	d) DETAIL=TRUE ;;
...
if [[ "$ZENITY" == "TRUE" ]] && [[ "$NOTIFY" == "TRUE" ]] ; then		# if both popup & notification specified; error.
	echo $RED">$STD Notification input error $RED[$STD -n & -z $RED]$STD"
	echo "please specify either -n or -z, not both"
	exit
fi
...
# - improve logging/parsing methods ?
# - include 'first seen / last seen' when using -M3 ?
# - add -z switch for zenity popup ?
# - add -n switch for zenity notifications ?
