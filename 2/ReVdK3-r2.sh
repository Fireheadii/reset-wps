...
echo -e "\e[36mПроверка, чтобы увидеть, если установлены следующие программы";
...
echo -e "\e[36m\e[40m\e[1m* Добро пожаловать: Мне нужно, чтобы проверить свой беспроводной интерфейс  *\e[0m";
echo -e	"\e[36m\e[40m\e[1m******************************************************\e[0m";
echo ; 
read -p "Какой беспроводной интерфейс вы будете использовать? e.g wlan0, wlan1 etc": WLAN; 
EXISTENCE_OF_WLAN=`airmon-ng|grep ''"$WLAN"|cut -f1`;
while [   -z "$WLAN" -o "$EXISTENCE_OF_WLAN" != "$WLAN" ]; do
echo -e "\e[31m\e[1mВы вводите беспроводной интерфейс, который не существует!\e[0m";
echo ;
read -p "Какой беспроводной интерфейс вы будете использовать? e.g wlan0, wlan1 etc": WLAN; 
EXISTENCE_OF_WLAN=`airmon-ng|grep ''"$WLAN"|cut -f1`;
done
