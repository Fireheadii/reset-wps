...
ifconfig $MON up
	...
	fi  # if [ $DEVMACTEST == n ] || [ $DEVMACTEST == Y ]; then
...
if [ $RCHOICE == n ] && [ $WRITEFILE == y ]; then

Eterm -g 80x10-1+400 --cmod "red" -T "REAVER" -e sh -c "reaver -i $MON -a -f -c $CHANNEL -b $TARGETAP -vv -x 60 -d 15 -T .5 --out-file=$LOGFILE-$PAD --mac=$VARMAC; bash" &

	fi

if [ $RCHOICE == y ] && [ $WRITEFILE == y ]; then

Eterm -g 80x10-1+400 --cmod "red" -T "Reaver" -e sh -c "reaver -i $MON -a -f -c $CHANNEL -b $TARGETAP -vv -x 60 -d 15 -T .5 -r $RX:$RY --out-file=$LOGFILE-$PAD --mac=$VARMAC; bash" &

	fi

if [ $RCHOICE == n ] && [ $WRITEFILE == n ]; then

Eterm -g 80x10-1+400 --cmod "red" -T "Reaver" -e sh -c "reaver -i $MON -a -f -c $CHANNEL -b $TARGETAP -vv -x 60 -d 15 -T .5 --mac=$VARMAC; bash" &

	fi

if [ $RCHOICE == y ] && [ $WRITEFILE == n ]; then

Eterm -g 80x10-1+400 --cmod "red" -T "Reaver" -e sh -c "reaver -i $MON -a -f -c $CHANNEL -b $TARGETAP -vv -x 60 -d 15 -T .5 -r $RX:$RY --mac=$VARMAC; bash" &
